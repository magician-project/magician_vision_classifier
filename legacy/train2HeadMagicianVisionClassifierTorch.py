...
def build_type_severity_maps(class_names):
    """Factorize combined 'class_<Type>Class<Severity>' labels into a (type, severity)
    pair for two-head training. The trailing severity letter (A/B/C) is stripped onto a
    shared GLOBAL severity axis; the remainder is the defect TYPE. 'class_clean' is a
    type. Classes with NO severity suffix (e.g. bare 'class_PositiveDent') get severity
    -1 = MASKED (excluded from the severity loss). Returns:
      type_of[c] -> type index for combined class c
      sev_of[c]  -> severity index (0=A,1=B,2=C) or -1 (masked)
      type_names -> list of type names in index order
      clean_type -> type index of clean (or None)
      num_sev    -> 3
    """
    import re
    sev_letter = {"A": 0, "B": 1, "C": 2}
    base_types, sevs = [], []
    for name in class_names:
        m = re.search(r'Class([ABC])$', name)
        if m:
            sevs.append(sev_letter[m.group(1)])
            base_types.append(name[:m.start()])
        else:
            sevs.append(-1)
            base_types.append(name)
    type_names = sorted(set(base_types))
    type_idx = {t: i for i, t in enumerate(type_names)}
    type_of = [type_idx[t] for t in base_types]
    clean_type = None
    for i, t in enumerate(type_names):
        if t in ("class_clean", "Clean"):
            clean_type = i
    return type_of, sevs, type_names, clean_type, 3
...
class Classifier(pl.LightningModule):
    ...
    def __init__(self,
        ...
        # Two-head (type + severity) experiment. Type head INCLUDES clean, so the
        # detection metric stays 1 - P_type(clean). Severity head is global A/B/C,
        # supervised only on tiles with a real severity (sev_of >= 0).
        self.two_head = bool(two_head)
        self.num_types = int(num_types)
        self.num_severities = int(num_severities)
        self.clean_type = int(clean_type) if clean_type is not None else 0
        self.type_loss_weight = float(type_loss_weight)
        self.severity_loss_weight = float(severity_loss_weight)
        if self.two_head:
            self.register_buffer('type_of', torch.as_tensor(type_of, dtype=torch.long))
            self.register_buffer('sev_of',  torch.as_tensor(sev_of,  dtype=torch.long))
        ...
        # Two-head: replace the backbone's single num_classes head with an Identity
        # (so it emits pooled features) and attach a TYPE head and a SEVERITY head.
        # Only the backbones we actually run two-head are wired here.
        if self.two_head:
            if ('custom' in self.type) or ('cnn' in self.type):
                feat_dim = int(self.final_dense_layer * 1.5)   # CustomCNN intermediate width
                self.model.out = nn.Identity()
            elif self.type == 'convnext_tiny':
                feat_dim = 768
                self.model.classifier[2] = nn.Identity()
            else:
                raise NotImplementedError(f"two_head path not wired for backbone '{self.type}'")
            self.type_head = nn.Linear(feat_dim, self.num_types)
            self.sev_head  = nn.Linear(feat_dim, self.num_severities)
        ...
        # In two-head mode the multiclass metrics score the TYPE head (num_types),
        # and a separate accuracy scores the severity head.
        metric_classes = self.num_types if self.two_head else num_classes
        self.accuracy  = Accuracy(task='MULTICLASS',  num_classes=metric_classes)
        self.recall    = Recall(task='MULTICLASS',    num_classes=metric_classes)
        self.precision = Precision(task='MULTICLASS', num_classes=metric_classes)
        self.auroc     = AUROC(task='MULTICLASS',     num_classes=metric_classes)
        ...
        if self.two_head:
            self.sev_accuracy = Accuracy(task='MULTICLASS', num_classes=self.num_severities)
    ...
    def forward(self, x):
        ...
        feats = self.model(x)
        if self.two_head:
            return self.type_head(feats), self.sev_head(feats)
        return feats

    def two_head_loss(self, type_logits, sev_logits, y):
        """Weighted TYPE focal loss + SEVERITY cross-entropy (masked to tiles with a
        real severity), plus the false-clean penalty applied to the TYPE head's clean
        probability. type_loss_weight > severity_loss_weight makes type errors cost more."""
        type_t = self.type_of[y]
        sev_t  = self.sev_of[y]
        l_type = self.criterion(type_logits, type_t)          # focal over the type classes
        m = sev_t >= 0
        l_sev = F.cross_entropy(sev_logits[m], sev_t[m]) if m.any() else type_logits.new_zeros(())
        loss = self.type_loss_weight * l_type + self.severity_loss_weight * l_sev
        if self.penalize_false_clean > 0.0:
            p = F.softmax(type_logits, dim=1)
            non_clean = (type_t != self.clean_type)
            if non_clean.any():
                p_clean = p[non_clean, self.clean_type]
                loss = loss + float(self.penalize_false_clean) * (-torch.log(1.0 - p_clean + 1e-8)).mean()
        return loss
    ...
    def training_step(self, batch, _batch_idx):
        ...
        if self.two_head:
            loss = self.two_head_loss(self.type_head(feats), self.sev_head(feats), y)
        else:
            y_hat = feats
    ...
    def validation_step(self, batch, _batch_idx):
        ...
        if self.two_head:
            type_logits, sev_logits = out
            loss = self.two_head_loss(type_logits, sev_logits, y)
            self.log('val_loss', loss, sync_dist=True)
            type_t = self.type_of[y]
            self.accuracy.update(type_logits, type_t)
            self.recall.update(type_logits, type_t)
            self.precision.update(type_logits, type_t)
            self.auroc.update(type_logits, type_t)
            sev_t = self.sev_of[y]; m = sev_t >= 0
            if m.any():
                self.sev_accuracy.update(sev_logits[m], sev_t[m])
            # Detection AUROC from the type head's clean probability (same semantics
            # as single-head 1 - P(clean), so val_detect_auroc stays comparable).
            probs = F.softmax(type_logits, dim=1)
            self.detect_auroc.update(1.0 - probs[:, self.clean_type],
                                     (type_t != self.clean_type).long())
            return loss
    ...
    def on_validation_epoch_end(self):
        ...
        if self.two_head:
            self.log('val_sev_accuracy', self.sev_accuracy.compute(), prog_bar=True, sync_dist=True)
            self.sev_accuracy.reset()
        self.accuracy.reset()
        self.recall.reset()
...
if __name__ == "__main__":
    ...
    # --- Two-head (type + severity) experiment wiring (config: "two_head": true) ---
    two_head = bool(config_json.get('two_head', False))
    th_type_of = th_sev_of = th_type_names = th_clean_type = None
    num_types = 0
    num_sev = 3
    if two_head:
        th_type_of, th_sev_of, th_type_names, th_clean_type, num_sev = build_type_severity_maps(class_names)
        num_types = len(th_type_names)
        print(f"[two-head] types ({num_types}): {th_type_names}")
        print(f"[two-head] clean_type={th_clean_type}  type_of={th_type_of}  sev_of={th_sev_of}")
    type_loss_weight     = float(config_json.get('type_loss_weight', 1.0))
    severity_loss_weight = float(config_json.get('severity_loss_weight', 0.3))
    ...
    classifier = Classifier(
                            ...
                            two_head=two_head,
                            num_types=num_types,
                            num_severities=num_sev,
                            type_of=th_type_of,
                            sev_of=th_sev_of,
                            clean_type=(th_clean_type if th_clean_type is not None else 0),
                            type_loss_weight=type_loss_weight,
                            severity_loss_weight=severity_loss_weight)

    if two_head:
        # Reconstruct a combined-class prediction from (type_pred, sev_pred) for the
        # confusion matrix. Clean type -> clean class for any severity; a (type,sev)
        # combo that does not exist as a real class falls back to that type's first class.
        pair_to_combined = {}
        type_fallback = {}
        for c in range(len(class_names)):
            t, s = th_type_of[c], th_sev_of[c]
            type_fallback.setdefault(t, c)
            if s >= 0:
                pair_to_combined[(t, s)] = c
        lut = [[0] * num_sev for _ in range(num_types)]
        for t in range(num_types):
            for s in range(num_sev):
                lut[t][s] = cleanClassID if t == th_clean_type else pair_to_combined.get((t, s), type_fallback[t])
        classifier._combine_lut = torch.tensor(lut, dtype=torch.long)
        classifier._two_head_clean_type = th_clean_type
    ...
    try:
        ...
        with torch.no_grad():
            for x, y in val_loader:
                ...
                out = classifier(x)
                if getattr(classifier, 'two_head', False):
                    type_logits, sev_logits = out
                    p = torch.softmax(type_logits, dim=1)
                    lut = classifier._combine_lut.to(x.device)
                    pr = lut[type_logits.argmax(dim=1), sev_logits.argmax(dim=1)]
                    mp = p.max(dim=1).values
                    y_true.extend(y.cpu().numpy())
                    y_pred.extend(pr.cpu().numpy())
                    y_maxp.extend(mp.cpu().numpy())
                    y_pcln.extend(p[:, classifier._two_head_clean_type].cpu().numpy())
                else:
