...
DistanceAverage, Light1-6 / lightDirection / lightNumber / lightConfidence, and
per-recording duration & framerate derived from dev_timestamp.
...
ids, extended to the rest of the metadata.
...
def bar(ax, labels, values, title, ylabel, rotate=90, top=None):
...
def report_split(h5_path, out_dir, cache_dir, split_name, limit=None):
    ...
    bar(ax, [c.replace("class_", "") for c in class_names], cls_counts,
        f"{split_name}: tiles per class", "tiles")
    fig.tight_layout()
    ...
    bar(axes[0], rec_names, tiles_per_rec, f"{split_name}: tiles per recording", "tiles", top=40)
    bar(axes[1], rec_names, frames_per_rec, f"{split_name}: frames per recording", "frames", top=40)
    fig.tight_layout()
    ...
    for ax, field in zip(axes.flat, dist_fields):
        ...
        ax.hist(vals, bins=60, color="#55A868")
    ...
    bar(axes[1, 0], num_labels,
        ...
        f"{split_name}: lightNumber", "frames", rotate=0)
    axes[1, 1].hist(confidence, bins=30, color="#C44E52")
